...
OUTPUT_EXCEL = Path("sedi_geocodificate.xlsx")
OUTPUT_CSV = Path("sedi_geocodificate.csv")

USER_AGENT = "sedi-operative-powerbi-geocoder/1.0"
...
def build_full_address(row: pd.Series) -> str:
    """Costruisce la query principale per Nominatim."""
...
def geocode_with_retry(geolocator: Nominatim, query: str) -> tuple[object, str]:
    """Esegue una richiesta Nominatim con retry e pausa tra i tentativi."""
    last_error = ""
...
def geocode_address(geolocator: Nominatim, row: pd.Series) -> dict:
    """Geocodifica una sede usando query completa e fallback semplificato."""
    ...
    if location:
        return {
            ...
            "Longitudine": location.longitude,
            "Indirizzo_Trovato": location.address,
            "Stato_Geocoding": "TROVATO_FALLBACK",
        }
    ...
    if location:
        return {
            ...
            "Longitudine": location.longitude,
            "Indirizzo_Trovato": location.address,
            "Stato_Geocoding": "TROVATO_COMUNE",
        }
...
def find_header_row(df_raw: pd.DataFrame) -> int | None:
    """Trova la riga che contiene le intestazioni reali della tabella."""
    expected = {keyword.lower() for keyword in HEADER_KEYWORDS}

    for index, row in df_raw.iterrows():
        values = {clean_value(value).lower() for value in row if clean_value(value)}
...
def build_dataframe_from_tab_column(df_raw: pd.DataFrame) -> tuple[pd.DataFrame, int]:
    """Gestisce file in cui una tabella tabulata e' finita in una sola colonna."""
    split_df = df_raw.iloc[:, 0].fillna("").astype(str).str.split("\t", expand=True)
    header_row = find_header_row(split_df)

    if header_row is None:
        raise ValueError(
            "Impossibile trovare la riga di intestazione anche dividendo la prima "
            "colonna per tabulazioni."
        )

    headers = split_df.iloc[header_row].astype(str).str.strip()
    df = split_df.iloc[header_row + 1 :].copy()
    df.columns = headers
...
def main() -> None:
    """Legge l'Excel, geocodifica le sedi e salva output Excel/CSV per Power BI."""
    ...
    found_count = result_df["Stato_Geocoding"].isin(
        ["TROVATO", "TROVATO_FALLBACK", "TROVATO_COMUNE"]
    ).sum()
    not_found_count = (result_df["Stato_Geocoding"] == "NON_TROVATO").sum()
    error_count = result_df["Stato_Geocoding"].astype(str).str.startswith("ERRORE").sum()

    print("\nRiepilogo geocoding")
    print("-------------------")
    print(f"Totale righe: {total_rows}")
    print(f"Indirizzi trovati: {found_count}")
    print(f"Indirizzi non trovati: {not_found_count}")
